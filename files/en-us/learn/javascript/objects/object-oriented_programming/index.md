...
Programação orientada o objetos é sobre modelagem de um sistema como uma de objetos, onde cada objeto representa algum aspecto particular de um sistema. Objetos contém funções(ou métodos) e dados. Um objeto fornece uma interface pública ao código que usa outro código que mantém privado, estato interno; Outras partes do sistema não precisam se preocupar com o que está acontecendo dentro do objeto. 

## Classes e instâncias

Quando modelamos um problema em termos de POO, criamos uma definição abstrata de representações de tipos de objetos que precisamos em nosso sistema. Por exemplo, se estamos modelamos uma escola, talvez queremos objetos representando professores. Cada professor tem algumas propriedades em comum: As propriedades são nomes e disciplinas que lecionam. Adicionalmente, cada professor tem certas coisas: Podem lançar as notas e se apresentar aos estudantes no início do período letivo, por exemplo.

Então `Professor` poderá ser uma **classe** em nosso sistema. A definição de classe pode listar dados e métodos que cada professor possui.

Em pseudocódigo, um `Professor` pode ser escrito assim:
...
Isso define uma classe `Professor`:

- duas propriedades de dados: `nome` e `ensina`
- dois métodos: `notas()` para inserir as notas e `apresentacao()` para se apresentar.
